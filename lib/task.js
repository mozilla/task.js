...
var supportsStarGenerators = false;
try { 
    Function("(function*(){})");
    supportsStarGenerators = true;
} catch (e) {}

if (!supportsLegacyGenerators && !supportsStarGenerators) {
    throw new Error("Generators are not supported in this environment.");
}
...
function runScheduledTask(task) {
    ...
    if (task.threadState === T_CANCELLED) {
        ...
        try {
            var result = send ? task.thread[sendMethod](result) : task.thread["throw"](result);
            if (result.done) {
                resolveTask(task, result.value);
            } else {
                handleResult(task, result.value || READY);
            }
        } catch (e) {
            rejectTask(task, e);
        }
