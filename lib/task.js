...
/** @public function(any) : Wait */
Wait.from = function(x) {
    while (!(x instanceof Wait)) {
        if (typeof x !== "object" || x === null || typeof x.toWait !== "function")
            return null;
        x = x.toWait();
    }
    return x;
...
/** @export class TaskWait(Task) is Wait<Uint32> */
function TaskWait(task) {
    this.task = task;
}

TaskWait.prototype = new Wait({
    onCancel: function() {
        this.task.stop();
    },
});
...
function started() {
    ...
    try {
        ...
        var now = Date.now();
        if (e === StopIteration) {
            this.taskWait.fulfill({ value: now, "throw": false });
        } else {
            this.uncaught = { value: e };
            this.taskWait.fulfill({ value: e, "throw": true });
        }
        shutdown(this);
    }
}
...
function cancelled() {
    ...
    this.taskWait.fulfill({ value: Date.now(), "throw": false });
    shutdown(this);
}
...
// FIXME: eliminate taskWait; Task should be <: Wait
...
function Task(thunk) {
    ...
    this.taskWait     /** @private TaskWait */           = new TaskWait(this);
}
