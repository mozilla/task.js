...
Evt.prototype = {
    ...
    fulfill: function(x, exception) {
        this.pending = { value: x, throw: !!exception };
        this.complete();
        this.notifyAll();
    },
...
JoinEvt.prototype = new Evt({
    ...
    notify: function(evt) {
        if (typeof(this.pendingEvts.remove(evt)) !== "undefined" && evt.readyState.throw)
            ...
            this.fulfill(this.evts.map(function(evt) { return evt.readyState }));
    }
});

// FIXME: rename to join and eliminate old join
function all() {
    var evts = [];
    for (var i = 0, n = arguments.length; i < n; i++)
        evts.push(arguments[i]);
...
function active() {
    ...
    try {
        if (!this.isBlocked()) { //(this.controlState === ACTIVE && !this.blockers.size) {
...
function shutdown(task) {
    ...
    while (waiting.length)
        waiting.pop().fulfill(task);
    //console.log(task + " shutting down");
    task.thread = null;
...
// FIXME: Task should implement Target (i.e., the notify() method)
Task.prototype = new Evt({
    // ===== queries =====
    state: function() {
    ...
    stop: function() {
        //console.log("preparing " + this + " to abort (scheduled? " + isReady(this) + ")");
    ...
    fulfill: function(blocker, result, throwResult) {
        ...
        //console.log("setting up fulfillment of " + this + " (scheduled? " + isReady(this) + ")");
        schedule(this);
    },
    join: function() {
        for (var i = 0, j = arguments.length; i < j; i++) {
            var evt = arguments[i];
            evt.waiting.push(this);
            this.blockers.set(evt, true);
        }
    },
    setCancelHandler: function(handler) {
        if (typeof handler !== "function")
            throw new TypeError("expected function, got " + handler);
        this.cancelHandlers.push(handler);
        return this.cancelHandlers.length - 1;
    },
    clearCancelHandler: function(id) {
        if (!(id in this.cancelHandlers))
            throw new Error("no registered cancel handler with id " + id);
        delete this.cancelHandlers[id];
    },
    isBlocked: function() {
        return !!(this.state === ACTIVE && this.blockers.size);
    },
    toString: function() {
