...
// TODO:
//  - add NEWBORN state and split start([x]) into start() and unpause([x])
...
//  - object-detect for WeakMap instead of ObjectMap
...
const ACTIVE  = 0,   // active (may or may not currently be executing)
      PAUSED  = 1,   // paused
      ABORTED = 2    // aborted but not yet done cleaning up
      CLOSED  = 3    // completely done
...
function setPending(task, result, throwResult) {
    if ((task.controlState === ACTIVE || task.controlState === PAUSED) &&
        (!task.pending || !task.pending.throw)) {
...
Task.prototype = {
    ...
    start: function(x) {
        ...
        if (this.newborn && x !== void 0)
            throw new Error("newborn task can only be started with undefined value");
    ...
    fulfill: function(blocker, result, throwResult) {
        ...
        if (this.controlState !== PAUSED && !this.blockers.size)
            schedule(this);
    },
    join: function() {
